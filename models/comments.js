  ...
  try {
    const insertResult= await new Promise((resolve, reject) => {
      connection.query(
        ...
        connection.query(
          'SELECT c.*, u.picture_id, u.name FROM comments c JOIN users u ON c.user_id = u.id WHERE c.comment_id = ? ',
          [insertedId],
          (err, results) => {
            if (err) {
...
const showComments = async (comment) => {
    try {
        return await new Promise((resolve, reject) => {
            connection.query(
                'SELECT c.*, u.picture_id, u.name FROM comments c JOIN users u ON c.user_id = u.id WHERE c.post_id = ? ORDER BY c.created_at ASC',
                [comment.postId],
                (err, results) => {
                    if (err) {
...
const editComment =async(comment)=>{
  try {
    ...
    return await new Promise((resolve, reject) => {
      const sql = 'UPDATE comments SET content = ? WHERE post_id = ? AND user_id = ? AND comment_id= ?';
      connection.query(sql, [comment.newContent, postID, ID,comment.commentId], (err, results) => {
        if (err) {
